...
if populate_db:  # populating the database
    ...
    PF1 = (
        PhysicsFight.objects.filter(fight_number=1)
        .filter(round_number=1)
        .filter(room=Room1)[0]
    )
    PF2 = (
        PhysicsFight.objects.filter(fight_number=2)
        .filter(round_number=1)
        .filter(room=Room1)[0]
    )
    PF3 = (
        PhysicsFight.objects.filter(fight_number=3)
        .filter(round_number=1)
        .filter(room=Room1)[0]
    )
