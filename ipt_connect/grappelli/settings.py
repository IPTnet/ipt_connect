...
AUTOCOMPLETE_SEARCH_FIELDS = getattr(
    settings, "GRAPPELLI_AUTOCOMPLETE_SEARCH_FIELDS", {}
)
...
SWITCH_USER_ORIGINAL = getattr(
    settings, "GRAPPELLI_SWITCH_USER_ORIGINAL", lambda user: user.is_superuser
)
...
SWITCH_USER_TARGET = getattr(
    ...
    lambda original_user, user: user.is_staff and not user.is_superuser,
)
