...
@override_settings(
    GRAPPELLI_SWITCH_USER_TARGET=lambda original_user, user: user.is_staff
...
class SwitchTests(TestCase):
    def setUp(self):
        ...
        self.superuser_2 = User.objects.create_superuser(
            "Superuser002", "superuser002@example.com", "superuser002"
        )
        ...
        self.editor_1.save()
        self.editor_2 = User.objects.create_user(
            "Editor002", "editor002@example.com", "editor002"
        )
    ...
    def test_switch_login(self):
        ...
        )  # redirect to login, FIXME: better testing
    ...
    def test_switch_superuser001_superuser002(self):
        ...
        response = self.client.get(
            "%s?redirect=%s"
            % (
                reverse("grp_switch_user", args=[target_user.id]),
