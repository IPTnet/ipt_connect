...
def participant_detail(request, pk):
    ...
    rounds = (Round.objects.filter(reporter=participant) | Round.objects.filter(
        reporter_2=participant) | Round.objects.filter(opponent=participant) | Round.objects.filter(
        reviewer=participant)).order_by('pf_number', 'round_number')
...
def team_detail(request, team_name):
    ...
    participants = Participant.objects.filter(team=team).filter(role='TM') | Participant.objects.filter(
        team=team).filter(role='TC')
...
def physics_fight_detail(request, pfid):
    ...
    for room in rooms:
        ...
        juryallgrades = [{'juryroundsgrades': gradesdico[jury], 'name': jury.name + " " + jury.surname} for jury in
                         gradesdico.keys()]
        print juryallgrades
...
def create_summary(roomrounds, teams_involved=None, finished=None):
    ...
    try:
        summary_grades = {team: [team.presentation_coefficients()[int(roomrounds[0].pf_number) - 1]] for team in
                          teams_involved}
        for team in teams_involved:
            for r in roomrounds:
                summary_grades[team].append(
                    # TODO: looks like this is not the fastest way!
                    team.get_scores_for_rounds(rounds=roomrounds.filter(round_number=r.round_number),
                                               include_bonus=False)[0]
                )
